...
- 三个后端的新建会话模式页面完全统一，默认使用设置中保存的 Agent 与权限模式；首次安装为 **Claude + Default**。
...
- 权限审批提供风险分级、影响范围与授权记录，活动时间线可追溯关键操作。
- 支持图片附件、语音输入、斜杠命令和 `@文件` 补全。
- 在输入框输入 `@` 可选择中文专业 Agent，并把角色说明注入当前任务。
- Codex 会话支持运行中追加指令（steer）、原生上下文压缩、目标、代码审查、分支和归档。
- 在 App 内管理 Codex Skills、Marketplace Plugins、MCP 服务器与 Apps 授权入口。
- 通过统一工作区查看改动文件、Markdown/语法高亮 diff、最近终端记录，并可进入远程终端。
- 运行中继续发送的提示词进入可管理队列，可立即追加或移除；复杂任务会聚合显示计划与子 Agent 进度。
- 每个项目可分别记住 Agent、模型、权限模式与推理强度，新会话自动套用项目偏好。
...
- 输入框上方：切换模型、思考强度和执行模式，并查看上下文用量；这些入口不再重复放在右上角菜单。
- 右上角 **⋯ → 目标**：设置目标、可选 Token 预算，以及暂停、继续、完成或清除目标。
- 右上角 **⋯ → 代码审查**：审查未提交改动、相对基础分支的改动、指定提交，或使用自定义审查说明。
- 右上角 **⋯ → 技能与插件**：启停 Codex Skills，查看 Marketplace 并安装或卸载 Plugins。
- 右上角 **⋯ → MCP 与 Apps**：查看 MCP 状态、重新加载配置、发起 OAuth 登录，并打开官方 App 连接页面。
- 会话列表可切换到“已归档”；会话快捷操作支持创建分支与归档/恢复。
- **设置 → Token 用量**：显示 Codex 本机官方快照明确提供的限额窗口、重置时间和可用的重置次数。没有返回的窗口不会由 CC Pocket 推测。

完整步骤和兼容性说明见 [使用指南](docs/USAGE.md)。
...
- [daemon artifact 部署与升级](docs/DAEMON-DEPLOYMENT.md)
- [服务器迁移与无锁定切换](docs/SERVER-MIGRATION.md)
