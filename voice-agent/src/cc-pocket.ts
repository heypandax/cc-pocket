...
const CC_POCKET_ROOT = join(__dirname, '../..'); // voice-agent 在 cc-pocket 内部
...
1. 手机连不上：
   - 检查 daemon 是否在运行（cc-pocket-daemon status）
   - 确认手机和电脑在同一网络或 relay 可达
   - 最常见原因：同时跑了两个 daemon，互相冲突
   - 用 update-local-daemon.sh 修复

2. 会话卡死/状态乱跳：
   - 也是多 daemon 冲突的典型症状
   - ps aux | grep cc-pocket-daemon 检查进程数

3. relay 断连：
   - 检查网络代理/VPN（TUN 代理可能拦截 relay 连接）
...
3. 确认 daemon 连上了 relay：
   检查 /status 的 relay 字段
4. 查看 daemon 日志 ~/Library/Logs/cc-pocket/daemon.err.log
5. 如果都不行，重置 daemon：
...
export function countDaemonProcesses(): number {
  try {
    const result = execSync(
      "ps aux | grep 'cc-pocket-daemon' | grep -v grep | wc -l",
      { encoding: 'utf-8' },
    ).trim();
    ...
    case 'check_cc_pocket_status': {
      ...
      if (!status.running) {
        return `CC Pocket daemon 状态：❌ 未运行\n` +
          `详情：${status.error}\n` +
          `建议：运行 cc-pocket-daemon run 启动 daemon，或用 cc-pocket-daemon status 查看详情`;
      }
