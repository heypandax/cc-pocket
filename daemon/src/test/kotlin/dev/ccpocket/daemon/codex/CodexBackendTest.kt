...
class CodexBackendTest {
    ...
    fun integrations_list_and_oauth_use_official_rpc() = runBlocking {
        ...
        val mcp = b.parse("""{"id":4,"result":{"data":[{"name":"docs","authStatus":"notLoggedIn","tools":{"search":{"name":"search","description":"Search docs","inputSchema":{}}},"resources":[{"name":"guide","uri":"docs://guide"}],"resourceTemplates":[{"name":"page","uriTemplate":"docs://{page}"}],"serverInfo":{"name":"docs","title":"Docs","version":"1"}}]}}""")
        val server = assertIs<AgentEvent.IntegrationsChanged>(mcp.single()).servers?.single()!!
        assertEquals(1, server.toolCount)
        assertEquals("Search docs", server.tools.single().description)
        assertEquals("docs://guide", server.resources.single().uri)
        assertEquals("docs://{page}", server.templates.single().uri)
