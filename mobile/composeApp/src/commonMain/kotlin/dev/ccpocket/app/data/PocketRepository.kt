...
class PocketRepository(private val scope: CoroutineScope, private val pinnedTo: PairedDaemon? = null) {
    ...
    fun disconnect() {
        ...
        directories.clear(); sessions.clear(); messages.clear(); activityEvents.clear()
        pendingImages.clear(); clearFileUploads(); clearBackgroundJobs()
    ...
    fun sendPrompt(text: String): Boolean {
        ...
        streaming.value = true
        workdir.value?.let { promptRetry = PromptRetry(outText, images, it, promptId, selectedAgents); promptResendArmed = false }
