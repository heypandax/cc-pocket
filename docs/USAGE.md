...
使用安装器安装过 daemon 后，日常升级只需在 daemon 所在电脑或服务器运行：
...
升级器会下载并校验最新 Release，切换到新版本，然后自动重启后台服务。身份密钥、已配对手机、Relay 地址和 Agent 路径配置都会保留，**不需要重新运行 `pair`**。

Linux 如果显示“installed”但自动重启失败，或服务仍运行旧版本，执行：
...
如果 shell 找不到命令，可改用 `$HOME/.local/bin/cc-pocket-daemon update`。macOS Homebrew 安装仍建议使用 `brew upgrade --cask ac54u-mobile/tap/cc-pocket`。手动部署、校验、回滚与日志检查见 [daemon 部署与升级](./DAEMON-DEPLOYMENT.md)。
...
⋯ 菜单 → **改动文件** 会打开统一的**工作区**：集中查看本会话改动、Markdown/代码 diff 和最近终端记录，也可直接进入远程终端。点开文件即可预览；看完 **←** 返回工作区继续浏览。

Agent 正在运行时仍可继续发送。新消息会进入输入框上方的待执行队列；点击**立即**可把它马上交给当前 Agent，点击**移除**会在电脑端真实取消该条等待消息。断线重连后队列以 daemon 保存的状态为准重新显示。

复杂任务执行期间，输入区上方会聚合显示当前计划和子 Agent 的完成/运行数量；详细工具卡仍保留在时间线中。

在项目的新建会话卡片中选择 Agent 和权限模式后，CC Pocket 会把 Agent、模型、权限模式及推理强度作为该项目偏好保存。以后从同一项目新建会话优先使用项目偏好；已有会话仍恢复自己的历史参数。
...
| 如何升级 daemon | 运行 `cc-pocket-daemon update`，再用 `cc-pocket-daemon --version` 确认；通常不需要重新配对 |
