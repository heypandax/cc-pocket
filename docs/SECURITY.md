...
> 适用范围：当前部署 `wss://relay.txx.app`（中转）。relay 完全开源、可自托管（见文末）。
...
Codex Skills、Plugins、MCP 和 Apps 操作发生在 daemon 主机当前登录的 Codex 环境中。CC Pocket 只通过端到端加密通道转发用户选择和显示 app-server 返回的状态：

- MCP OAuth 在系统浏览器与提供方页面完成；CC Pocket 不接收或保存账号密码、访问令牌或 OAuth 凭据。
- 安装/卸载 Plugin、启停 Skill 会修改 daemon 主机上的 Codex 配置，因此只应在自己控制的主机和账号上执行。
- Apps 连接使用 Codex 返回的官方安装地址；打开前应核对目标服务、权限范围和账号。
- Token 用量与限额来自本机官方快照/app-server；缺失窗口不会由 CC Pocket 推测，账号侧最终结果以官方面板为准。
...
请始终通过 GitHub 私密漏洞报告渠道提交安全问题，不要公开未修复漏洞。
